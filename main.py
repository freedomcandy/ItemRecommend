...
#use NaiveBayes  to predict category_id
class ItemRecommendNaiveBayesHandle(tornado.web.RequestHandler):
    def post(self):
        feature_train = eval(self.get_argument('tapID', '[]' ))
        label_train = eval(self.get_argument('tapedID', '[]' ))
        print(self.get_argument('tapID', '[]' ))
        ...
        if len(feature_train) < 1:
            ...
            return
        feature_test = label_train[len(label_train)-1]
        predict_data = nb.NaiveBayes().processDataWithGaussianNB(feature_train, label_train, feature_test)
        print(predict_data)
        result_data = {'categoryIds':predict_data}
        self.write(json.dumps(result_data))

    def get(self):
        self.write('NaiveBayes GET OK')
...
#         (r'/itemRec', ItemRecommendRegressionHandler)
    application = tornado.web.Application([
        ...
        (r'/itemRecNB', ItemRecommendNaiveBayesHandle)
        ], autoreload=True)
